...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const revenueResult = await orderModel.aggregate([
      { $match: { status: { $in: ["delivered", "shipped", "confirmed", "paid"] } } },
    ...
    const topProducts = await productModel
      .find({})
      .sort({ createdAt: -1 }) // Bạn có thể sửa logic này sau để lấy theo soldQuantity
      .limit(5);
    ...
    // Calculate growth percentages (Mock data - giữ nguyên theo code của bạn)
    const stats = {
      totalProducts,
      totalOrders,
      totalUsers,
    ...
    const todaysSalesResult = await orderModel.aggregate([
      {
        $match: {
          ...
          // Thêm 'paid' vào để tính cả các đơn đã thanh toán online
          status: { $in: ["delivered", "shipped", "confirmed", "paid"] }, 
        },
      },
      { $group: { _id: null, totalSales: { $sum: "$amount" } } },
    ]);

    const todaysSales = todaysSalesResult.length > 0 ? todaysSalesResult[0].totalSales : 0;
